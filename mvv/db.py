...
``DATABASE_URL`` at a Postgres instance and ``db/schema.postgres.sql`` is
applied verbatim. SQLite is supported as a zero-setup backend for local runs
...
— so aggregation happens in Python over ``Decimal``. See mvv/money.py.
"""
...
SCHEMA_POSTGRES = REPO_ROOT / "db" / "schema.postgres.sql"
SCHEMA_SQLITE = REPO_ROOT / "db" / "schema.sqlite.sql"
...
class Database:
    ...
    def schema_path(self) -> Path:
        return SCHEMA_POSTGRES if self.dialect == "postgres" else SCHEMA_SQLITE
