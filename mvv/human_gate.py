...
from .db import Database, to_datetime, utcnow_iso
...
class Approval:
    ...
    def from_row(cls, row: dict) -> "Approval":
        ...
        return cls(
            ...
            decided_at=to_datetime(row.get("decided_at")),
...
Recommended action: KILL or CONTINUE?
  ...
  mvv approve {approval_id} --by you      # KILL
  mvv reject  {approval_id} --by you      # CONTINUE
  mvv show {experiment_id}                # REVIEW_DATA
"""
...
you decide:
  ...
  mvv approve {approval_id} --by you
  mvv reject  {approval_id} --by you
"""
...
class HumanGate:
    ...
    def request_kill(self, experiment, reason: str) -> Approval:
        ...
        if approval.is_pending and approval.decided_at is None:
            self.notifier.send(
    ...
    def find_pending(self, action: str, experiment_id: Optional[int]) -> Optional[Approval]:
        if experiment_id is None:
            row = self.db.query_one(
                "SELECT * FROM approvals WHERE action = ? AND experiment_id IS NULL "
                "AND status = ? ORDER BY id DESC LIMIT 1",
                (action, ApprovalStatus.PENDING.value),
            )
        else:
            row = self.db.query_one(
                "SELECT * FROM approvals WHERE action = ? AND experiment_id = ? "
                "AND status = ? ORDER BY id DESC LIMIT 1",
    ...
    # -- decisions --------------------------------------------------------
    def decide(
        self, approval_id: int, decision: str, decided_by: str = "operator", notes: str = ""
    ) -> Approval:
    ...
    def approve(self, approval_id: int, decided_by: str = "operator", notes: str = "") -> Approval:
        return self.decide(approval_id, "approve", decided_by, notes)

    def reject(self, approval_id: int, decided_by: str = "operator", notes: str = "") -> Approval:
        return self.decide(approval_id, "reject", decided_by, notes)
