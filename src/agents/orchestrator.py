...
from ..agents.cash_ledger import CashLedger, InsufficientCash
...
from ..agents.metrics import ManualMetricsProvider, MetricsProvider
from ..models import CashCategory, Experiment, Status
from ..agents.monitoring import HealthMonitor, format_alerts
from ..money import D, fmt_money, money
from ..notifications import Notifier, build_notifier
from ..agents.scout import Scout, build_scout
...
class Orchestrator:
    ...
    def _apply_kill_decisions(self, report: TickReport) -> None:
        ...
        for approval in self.db.query(
            "SELECT * FROM human_approvals WHERE action = ? AND status IN (?, ?) "
            "ORDER BY id",
            (
            ...
            if approval["status"] == ApprovalStatus.APPROVED.value:
                self.ledger.kill(
                    exp_id, exp.pending_kill_reason, decided_by=approval.get("approved_by") or "operator"
                )
    ...
    def _apply_scale_decisions(self, report: TickReport) -> None:
        ...
        for approval in self.db.query(
            "SELECT * FROM human_approvals WHERE action IN (?, ?) AND status = ? ORDER BY id",
            (
                ApprovalAction.SCALE_AD_SPEND.value,
        ...
        for exp in self.ledger.get_active():
            ...
            if self.config.gate.block_on_approval:
                ...
                if decided.is_approved:
                    self.ledger.kill(exp.id, reason, decided_by=decided.approved_by or "operator")
