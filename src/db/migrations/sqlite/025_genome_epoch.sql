...
-- and the benchmark-free fitness. Idempotent — `INSERT OR IGNORE` on a primary
-- key, so re-running `init-db` neither duplicates nor relabels, and a row that
-- epoch 2 has already claimed is left alone.
INSERT OR IGNORE INTO genome_epoch (genome_id, epoch, reason)
SELECT id, 1,
